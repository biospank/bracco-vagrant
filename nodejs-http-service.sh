#!/bin/bash
#
# http-server          Start/Stop any shell script
#
# chkconfig: 345 95 65
# description: http-server
# processname: http-serverd
#
...
HTTPSERVER_SCRIPT_DIR=/home/vagrant

# Construct the command the will cd into the right directory, and invoke the script
#HTTPSERVER_COMMAND="cd $HTTPSERVER_SCRIPT_DIR && ./http_server.sh 'param1' $PARAM2"
HTTPSERVER_COMMAND="cd $HTTPSERVER_SCRIPT_DIR && http-server '/vagrant/dist'"

# How can the script be identified if it appears in a 'ps' command via grep?
#  Examples to use are 'java', 'python' etc.
HTTPSERVER_PROCESS_TYPE=nodejs

# Where to write the log file?
HTTPSERVER_SVC_LOG_FILE=$HTTPSERVER_SCRIPT_DIR/http-server.log
...
PID_FILE=$HTTPSERVER_SCRIPT_DIR/http-server.pid

# Load system specific optional arguments
# Create and populate this file with machine specific settings
if [ -f /etc/sysconfig/http-serverd ]; then
    . /etc/sysconfig/http-serverd
fi
...
start() {
    if [ "${PID}" != "" ]; then
        ...
        if [ -a /proc/${PID} ]; then
            ...
            ps aux | grep ${PID} | grep $HTTPSERVER_PROCESS_TYPE >> /dev/null
            # If it is a process of the right type assume that it is http-server and just exit
            # otherwise remove the subsys lock file and start http-server
            if [ "$?" = "0" ]; then
                exit 1
            else
                echo "http-server lock file still exists, removing..."
                rm /var/lock/http-serverd
            fi
        else
            # The process running as pid $PID is not a process of the right type, remove subsys
            # lock and start http-server
            echo "http-server lock file still exists, removing..."
            rm /var/lock/http-serverd
        fi
    fi
    echo -n "Starting http-server: "
    su - $SCRIPT_USER -c "/bin/sh -c \"$HTTPSERVER_COMMAND > $HTTPSERVER_SVC_LOG_FILE 2>&1\"" & RETVAL=$?
    sleep 3
    touch /var/lock/http-serverd
    exit 0
}
...
stop() {
    ...
    if [ "${PID}" != "" ]; then
        ...
        for i in {1..30}
        do
            if [ -n "`ps aux | grep $HTTPSERVER_PROCESS_TYPE | grep http-server `" ]; then
                sleep 1 # Still running, wait a second.
                echo -n .
            else
                # stopped
                rm -f /var/lock/http-serverd
                rm -f $PID_FILE
                echo
                exit 0
            fi
